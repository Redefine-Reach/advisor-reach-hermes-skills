...
Gives the customer a Dave Ramsey-style retirement calculator web page — five inputs
(ages, savings, monthly contribution, expected return), a live projected nest egg and a
growth chart — from a locked AdvisorReach-branded template in `assets/`, delivered as a
link via `present-file` under the fixed name `Retirement-Calculator.html` (or on the
customer's own domain via `publish-site`). Change requests ("make the default return
8%", "add a field") are edits to the agent's working copy, re-delivered under the same
name so the link never changes. The template's math has a hermetic test:
`node --test tests/`.
